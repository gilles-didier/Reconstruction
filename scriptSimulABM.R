...
muMax <- 10
nMM <- 21
muM <- seq(-muMax, muMax, length=nMM)
#
...
####################################################
# MODIF fonctions PARADIS
####################################################



rTraitCont2 <- function (phy, model = "BM", sigma = 0.1, mu=0, alpha = 1, theta = 0, ancestor = FALSE, root.value = 0, ...) 
{
    ...
    des <- phy$edge[, 2]
    el <- phy$edge.length
    foo2 <- function(x,l,m, s)
	{
	x + rnorm(1, mean=m*l, sd=s*sqrt(l)) 
	}
    if (model=="ABM") 
    {
	for (inda in N:1) x[des[inda]] <- foo2(x[anc[inda]], el[inda], mu, sigma)
    }
    else {
        ...
        .C("rTraitCont", as.integer(model), as.integer(N), as.integer(anc - 
            1L), as.integer(des - 1L), el, as.double(sigma), 
            as.double(alpha), as.double(theta), x, DUP = FALSE, 
...
ValMuRec <- matrix(0, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ValMuRec) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
#
ValSigmaRec <- matrix(0, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ValSigmaRec) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
#
ErreursABMnrj <- matrix(NA, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ErreursABMnrj) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
#
ErreursABMnrjM <- matrix(NA, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ErreursABMnrjM) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
#
ErreursABMks <- matrix(NA, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ErreursABMks) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
#
ErreursABMmse <- matrix(NA, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ErreursABMmse) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
#
ErreursABMbiaisG <- matrix(NA, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ErreursABMbiaisG) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
#
ErreursABMbiaisAbsG <- matrix(NA, nrow=N*nSM*nMM, ncol=nMe*nN+2)
colnames(ErreursABMbiaisAbsG) <- c(paste(rep(Methodes, each=nN), nomNoeud, sep="_"), "muM", "sigmaM")
...
# 
	...
	muAcc <- c(m0, muM[i] * Tau)
	sigmaAcc <- c(v0, sigmaM[j]^2 * Tau)
	V2Acc <- diag(sigmaAcc)
	CABM <- calculeC("ABM", arbis)
...
#
	...
	while (k <= N)
	{
	simul <- rTraitCont2(arbre, model="ABM", mu=muM[i], sigma=sigmaM[j], ancestor=T,  root.value=z0)
...
#
	indL <- (i-1)*nSM*N + (j-1)*N +k
	ValMuRec[indL, ((nMe*nN+1):(nMe*nN+2)) ] <- c(muM[i], sigmaM[j])
	ValSigmaRec[indL, ((nMe*nN+1):(nMe*nN+2)) ] <- c(muM[i], sigmaM[j])
	#
	#
...
# GLSOU, GLSABM
	...
	#
	ErreursABMnrj[indL, 1:(nMe*nN)] <- NRJ(muR=ValMuRec[indL, (1:(nMe*nN)) ], muT=muTheo, sigma2R=ValSigmaRec[indL, (1:(nMe*nN))]^2, sigma2T=sigmaTheo, loiR=lois, ddl=nN)
	ErreursABMnrj[indL, (nMe*nN+1) ] <- muM[i]
	ErreursABMnrj[indL, (nMe*nN+2) ] <- sigmaM[j]
	#
	ErreursABMnrjM[indL, 1:(nMe*nN)] <- 2 * sqrt(sigmaTheo) * (sqrt(2) - 1) /sqrt(pi)
	ErreursABMnrjM[indL, (nMe*nN+1) ] <- muM[i]
	ErreursABMnrjM[indL, (nMe*nN+2) ] <- sigmaM[j]
	#
	ErreursABMks[indL, 1:(nMe*nN)] <- KS(muR=ValMuRec[indL, (1:(nMe*nN)) ], muT=muTheo, sigma2R=ValSigmaRec[indL, (1:(nMe*nN))]^2, sigma2T=sigmaTheo, loiR=lois, ddl=nN)
	ErreursABMks[indL, (nMe*nN+1) ] <- muM[i]
	ErreursABMks[indL, (nMe*nN+2) ] <- sigmaM[j]
	#
	ErreursABMmse[indL, 1:(nMe*nN)] <- sigmaTheo + (muTheo-ValMuRec[indL, (1:(nMe*nN)) ])^2
	ErreursABMmse[indL, (nMe*nN+1) ] <- muM[i]
	ErreursABMmse[indL, (nMe*nN+2) ] <- sigmaM[j]
	#
	ErreursABMbiaisG[indL, 1:(nMe*nN)] <- muTheo - ValMuRec[indL, (1:(nMe*nN)) ]
	ErreursABMbiaisG[indL, (nMe*nN+1) ] <- muM[i]
	ErreursABMbiaisG[indL, (nMe*nN+2) ] <- sigmaM[j]
	#
	ErreursABMbiaisAbsG[indL, 1:(nMe*nN)] <- NRJ(muR=ValMuRec[indL, (1:(nMe*nN)) ], muT=muTheo, sigma2R=rep(0,(nMe*nN)), sigma2T=sigmaTheo, loiR=rep("Norm", (nMe*nN)), ddl=nN)
	ErreursABMbiaisAbsG[indL, (nMe*nN+1) ] <- muM[i]
	ErreursABMbiaisAbsG[indL, (nMe*nN+2) ] <- sigmaM[j]
...
#
#
write.csv2(ErreursABMnrj, "NRJmodABM.csv", quote=F, row.names=F)
write.csv2(ErreursABMnrjM, "NRJMmodABM.csv", quote=F, row.names=F)
write.csv2(ErreursABMks, "KSmodABM.csv", quote=F, row.names=F)
write.csv2(ErreursABMmse, "MSEmodABM.csv", quote=F, row.names=F)
write.csv2(ErreursABMbiaisG, "BGmodABM.csv", quote=F, row.names=F)
write.csv2(ErreursABMbiaisAbsG, "BABSGmodABM.csv", quote=F, row.names=F)
